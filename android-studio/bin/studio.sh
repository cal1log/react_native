...
if [ -z "$JDK" -a -s "${XDG_CONFIG_HOME:-$HOME/.config}/Google/AndroidStudio4.2/studio.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.AndroidStudio4.2/config/studio.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
for VM_OPTIONS_FILE in \
    ...
    ${XDG_CONFIG_HOME:-$HOME/.config}/Google/AndroidStudio4.2/studio$BITS.vmoptions \
    $IDE_HOME.vmoptions \
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=AndroidStudio4.2 \
